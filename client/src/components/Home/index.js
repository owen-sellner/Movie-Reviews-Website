// Import React
import React, { Component } from 'react';
// Import MUI Elements
import Grid from "@material-ui/core/Grid";
import Typography from '@material-ui/core/Typography';
import Select from '@material-ui/core/Select';
import TextField from '@material-ui/core/TextField';
import Radio from '@material-ui/core/Radio';
import RadioGroup from '@material-ui/core/RadioGroup';
import FormControlLabel from '@material-ui/core/FormControlLabel';
import FormControl from '@material-ui/core/FormControl';
import FormLabel from '@material-ui/core/FormLabel';
import MenuItem from '@material-ui/core/MenuItem';
import Button from '@material-ui/core/Button';

const colSize = 1;

const serverURL = "http://ov-research-4.uwaterloo.ca:3103";

// Parent Component
function Review() {
    
    const [selectedMovie, setSelectedMovie] = React.useState([]);
    const [enteredTitle, setEnteredTitle] = React.useState([]);
    const [enteredReview, setEnteredReview] = React.useState([]);
    const [selectedRating, setSelectedRating] = React.useState([]);

    const handleSubmit = () => {
        setSelectedMovie(selectedMovie => selectedMovie.concat(newMovie))
        setEnteredTitle(enteredTitle => enteredTitle.concat(newTitle))
        setEnteredReview(enteredReview => enteredReview.concat(newReview))
        setSelectedRating(selectedRating => selectedRating.concat(newRating))
    }

    const [newMovie, setNewMovie] = React.useState('');
    const [newTitle, setNewTitle] = React.useState('');
    const [newReview, setNewReview] = React.useState('');
    const [newRating, setNewRating] = React.useState('');

    return(
        <div style={{margin: 15}}>
            {/* Creates a column grid for the body of the page */}
            <Grid container
                direction="column"
                justifyContent="space-evenly"
                alignItems="center"
            >
                {/* Page Title */}
                <Grid item>
                    <Typography variant="h3">
                        <b>Movie Reviews</b>
                    </Typography> 
                </Grid>

                {/* Review Form */}
                <Grid container
                    alignContent="center"
                    spacing={2} 
                    direction="row"
                >   
                    {/* Row 1 */}
                        <Grid item xs={4*colSize}></Grid>
                        <Grid item xs={4*colSize}>
                            <Typography variant="h4">
                                New Review
                            </Typography> 
                        </Grid>
                        <Grid item xs={12-8*colSize}></Grid>
                    {/* Row 2 */}
                        <Grid item xs={4*colSize}></Grid>
                        <Grid item xs={colSize}>
                            <Typography variant="h6">Select Movie:</Typography>
                        </Grid>
                        <Grid item xs={3*colSize}>
                            <MovieSelection />
                        </Grid>
                        <Grid item xs={12-8*colSize}></Grid>
                    {/* Row 3 */}
                        <Grid item xs={4*colSize}></Grid>
                        <Grid item xs={colSize}>
                            <Typography variant="h6">Enter Title:</Typography>
                        </Grid>
                        <Grid item xs={3*colSize}>
                            <ReviewTitle />
                        </Grid>
                        <Grid item xs={12-8*colSize}></Grid>
                    {/* Row 4 */}
                        <Grid item xs={4*colSize}></Grid>
                        <Grid item xs={colSize}>
                            <Typography variant="h6">Enter Review:</Typography>
                        </Grid>
                        <Grid item xs={3*colSize}>
                            <ReviewBody />
                        </Grid>
                        <Grid item xs={12-8*colSize}></Grid>
                    {/* Row 5 */}
                        <Grid item xs={4*colSize}></Grid>
                        <Grid item xs={colSize}>
                            <Typography variant="h6">Select Rating:</Typography>
                        </Grid>
                        <Grid item xs={3*colSize}>
                            <ReviewRating />
                        </Grid>
                        <Grid item xs={12-8*colSize}></Grid>
                    {/* Last Row */}
                        <Grid item xs={5*colSize}></Grid> 
                        <Grid item xs={2*colSize}>
                            <Button fullWidth 
                                variant="contained" 
                                color="primary" 
                                size="large" 
                                onClick={handleSubmit}
                                disableElevation>Submit</Button>
                        </Grid>
                </Grid>

                {/* User Results */}
                <Grid item>
                    <Typography variant="h4">
                        - - - - - - - - - - - - - - - - - - - -
                    </Typography> 
                </Grid>

            </Grid>
        </div>
    );
}

// Component for movie selection (Select)
function MovieSelection() {
    return(
        <div>
            <FormControl fullWidth>
                <FormLabel>Select Movie</FormLabel>
                <Select fullWidth id="movie-select" variant="outlined" onChange={event => setNewMovie(event.target.value)}>
                    <MenuItem value="Morbius">Morbius</MenuItem>
                    <MenuItem value="Morbius 2">Morbius 2</MenuItem>
                    <MenuItem value="Morbius 3">Morbius 3</MenuItem>
                    <MenuItem value="Morbius 4">Morbius 4</MenuItem>
                    <MenuItem value="Morbius 5">Morbius 5</MenuItem>
                </Select>
            </FormControl>
        </div>
    );
}

// Component for review title (Single-line Textbox)
function ReviewTitle() {
    return(
        <div>
            <TextField fullWidth id="review-title" label="Review Title" variant="outlined" onChange={event => setNewTitle(event.target.value)}/>
        </div>
    );
}

// Component for review text (Multi-line Textbox)
function ReviewBody() {
    return(
        <div>
            <TextField multiline fullWidth rows={5} id="review-body" label="Review Body" variant="outlined" onChange={event => setNewReview(event.target.value)}/>
        </div>
    );
}

// Component for User Score (Radio Group)
function ReviewRating() {
    return(
        <div>
            <FormControl>
            <FormLabel>Rating</FormLabel>
            <RadioGroup row={true} onChange={event => setNewRating(event.target.value)}>
                <FormControlLabel value="1" control={<Radio />} label="1"/>
                <FormControlLabel value="2" control={<Radio />} label="2"/>
                <FormControlLabel value="3" control={<Radio />} label="3"/>
                <FormControlLabel value="4" control={<Radio />} label="4"/>
                <FormControlLabel value="5" control={<Radio />} label="5"/>
            </RadioGroup>
            </FormControl>
            
        </div>
    );
}

export default Review;